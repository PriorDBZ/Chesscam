...
2. **Détection des cases modifiées** : pour chaque image stable (pas de main
   dans le champ), on compare la couleur moyenne et la texture de chaque case
   avec la dernière position validée.
...
- Éclairage **constant** (pas d'ombre qui balaie le plateau, pas de soleil
  direct changeant).
- Retirer la main du champ entre les coups (au moins ~1 s de plateau immobile
  après chaque coup).
...
- `FRAME_INTERVAL_US` (500 000) : fréquence d'échantillonnage.
